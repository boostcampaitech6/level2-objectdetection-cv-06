...
class MysqlClientManager(MysqlManager):
    ...
    def update(self) -> tuple[bool, int]:
        complete = False
        message_id = -1
        receiver_condition = f"receiver = '{CAMPER_NAME}' OR receiver = 'any'"
        seclect_query = f"SELECT * FROM message WHERE pushed = 0 AND ({receiver_condition}) ORDER BY id ASC LIMIT 1 FOR UPDATE;"
        print(seclect_query)
...
class MysqlCamperManager(MysqlManager):
    ...
    def message(self, message_id: Optional[int] = None) -> list[dict[str:any]]:
        result = []
        sql = (
            f"SELECT * FROM message WHERE id = {message_id}"
            if message_id != None
            else "SELECT * FROM message"
        )
        ...
        result = [
            ...
            for value in self.session.fetchall()
        ]
        self.close()

        return result

    def delete(self, message_id: Optional[int] = None) -> list[dict[str:any]]:
        result = []
        ...
        result.append({"message": "삭제 성공", "deleted_id": message_id})
        return result

    def error_message(self, message_id: Optional[int] = None) -> list[dict[str:any]]:
        result = []
        sql = (
            f"SELECT * FROM error_message WHERE id = {message_id}"
            if message_id != None
            else "SELECT * FROM error_message"
        )
