...
if __name__ == "__main__":
    ...
    pred_serial = dir_name + "_" + datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    submission_file = os.path.join(pred_result_dir, f"{pred_serial}.csv")
    os.makedirs(pred_result_dir, exist_ok=True)

    annotation_dir = yaml["test_annotation_dir"]
    data_dir = yaml["test_dir"]

    check_point = [p for p in os.listdir(target_dir) if p.startswith("best")][0]
    check_point_path = os.path.join(target_dir, check_point)
