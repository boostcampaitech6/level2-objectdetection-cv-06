...
def main():
    ...
    for fold_index in range(yaml["kfold_splits"]):
        if fold_index == 0 or fold_index == 1:
            continue
        # load config
        ...
        cfg.train_dataloader.dataset.ann_file = os.path.join(
            train_annotation_dir, f"train_dino_pseudo_fold_{fold_index}.json"
        )
        ...
        cfg.val_dataloader.dataset.ann_file = os.path.join(
            val_annotation_dir, f"val_dino_pseudo_fold_{fold_index}.json"
        )
