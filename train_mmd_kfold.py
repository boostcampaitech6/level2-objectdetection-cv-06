...
def main():
    ...
    for fold_index in range(yaml["kfold_splits"]):
        ...
        cfg.train_dataloader.dataset.ann_file = os.path.join(train_annotation_dir, f"train_fold_{fold_index}.json")

        # dataset val
        cfg.val_dataloader.dataset.metainfo = dict(classes=yaml["classes"])
        cfg.val_dataloader.dataset.data_root = data_dir
        cfg.val_dataloader.dataset.ann_file = os.path.join(val_annotation_dir, f"val_fold_{fold_index}.json")
        ...
        # evaluator
        cfg.val_evaluator.ann_file = os.path.join(val_annotation_dir, f"val_fold_{fold_index}.json")
