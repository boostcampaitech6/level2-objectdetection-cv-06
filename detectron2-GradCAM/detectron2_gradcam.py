...
class Detectron2GradCAM:
    ...
    def __init__(
        self, config_file, cfg_list, img_path, root_dir=None, custom_dataset=None
    ):
        ...
        if custom_dataset:
            register_coco_instances(
                custom_dataset, {}, root_dir + "train.json", root_dir
            )
            cfg.DATASETS.TRAIN = (custom_dataset,)
            MetadataCatalog.get(custom_dataset).thing_classes = [
                "General trash",
                "Paper",
                "Paper pack",
                "Metal",
    ...
    def _set_input_image(self, img_path):
        ...
        transformed_img = transform_gen.get_transform(self.image).apply_image(
            self.image
        )
        self.input_tensor = torch.as_tensor(
    ...
    def get_cam(self, target_instance, layer_name, grad_cam_instance):
        ...
        with grad_cam as cam:
            cam, cam_orig, output = cam(
                input_image_dict, target_instance=target_instance
            )
    ...
    def get_output_dict(self, cam, output, target_instance):
        ...
        image_dict["label"] = MetadataCatalog.get(
            self.cfg.DATASETS.TRAIN[0]
        ).thing_classes[output["instances"].pred_classes[target_instance]]
        return image_dict
